...
function echo_nav_end() {
	echo '
	<li class="nav-item">
    	<a class="nav-link btn btn-info" href="/project3/control_panel.php?table=wj18sawy_pages">Control Panel</a>
		</li>
...
function echo_nav($short_title, $current_page) {
	...
	$pages_level1 = run_query("SELECT pageid, title FROM wj18sawy_pages WHERE parent = 'home'");
	...
	while ($page_lv1 = $pages_level1->fetch_assoc()) {
		...
		$pages_level2 = run_query("SELECT pageid, title FROM wj18sawy_pages WHERE parent = '".$pageid."'");
